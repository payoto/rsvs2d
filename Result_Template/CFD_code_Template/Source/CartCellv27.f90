...
!     6) Final geometry - volumes, then compress lists and dump mesh    
     ...
     &  edgenew,blankcon
      ...
      real,allocatable,dimension(:,:)::grd,surfpt,grdnew
...
!       Dump the split cells for viewing
      ...
      if(blanking(i).eq.0)then
        ...
        if((edge(i,3).gt.0))then
          if(agg(edge(i,3)).ne.0)then
            edge(i,3)=agg(edge(i,3))
            !ncellfinal=ncellfinal-1
          endif
        endif
        if((edge(i,4).gt.0))then
          if(agg(edge(i,4)).ne.0)then
...
!           Cell has an alias, so use that                  
      ...
      volnew=0.0
      do i=1,nedgenewfinal
        midpt=0.5*(grd(edgecompnew(i,1),:)+grd(edgecompnew(i,2),:))
        dx=grd(edgecompnew(i,2),1)-grd(edgecompnew(i,1),1)
        dy=grd(edgecompnew(i,2),2)-grd(edgecompnew(i,1),2)
        contrib=0.5*(midpt(1)*(dy)+midpt(2)*(-dx))
...
!     Dump the mesh out
      write(GU,*) ncellfinal,nedgenewfinal,nptnew
      do i=1,nedgenewfinal
        write(GU,*) (edgecompnew(i,j),j=1,4)
      enddo
      do i=1,nptnew
        write(GU,*) i,(grd(i,j),j=1,2)
      enddo
      do i=1,ncellfinal
        write(GU,*) volnew(i)
      enddo
...
!      real,intent(in),dimension(npt,2)::grdloc
      ...
      integer,allocatable,dimension(:)::nmeet,colour
      integer::loopchecks
...
!          area=area+areainc
      ...
      deallocate(connectpt,nmeet,colour)
