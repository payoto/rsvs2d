...
program cart
	...
	&  edgenew,blankcon
	double precision, allocatable,dimension(:)::edgelength
	integer, allocatable,dimension(:)::edgeindex
	integer,allocatable,dimension(:)::dir,nbound,issurfcut,cutof
	double precision,allocatable,dimension(:,:)::grdout,grd,surfpt,grdnew
	double precision,allocatable,dimension(:,:)::intsct
	double precision,dimension(2)::botleft,topright,temp
	...
	double precision,dimension(2)::bigpos,pt,ed_a,ed_b 
	double precision::val,st,nd,test,pi,shiftx,shifty
	...
	double precision,dimension(2)::start,next
	double precision,allocatable,dimension(:)::array,volume,vol
	...
	double precision,allocatable,dimension(:,:)::grdloc
	integer,allocatable,dimension(:,:)::edgecomp,edgecompnew
	double precision,allocatable,dimension(:)::volnew
	...
	double precision,allocatable,dimension(:,:)::sumgrd
	...
	integer::verbfile
	double precision::volinit,contrib,dx,dy,volrat
	double precision,dimension(2)::midpt
	integer::nptout
...
!     are in or out, the challenge here is to know which cell we are in.
	...
	allocate(edgelength(nedgenew))
	allocate(edgeindex(nedgenew))

	do i=1,nedgenew
		dx=grd(edge(i,2),1)-grd(edge(i,1),1)
		dy=grd(edge(i,2),2)-grd(edge(i,1),2)
		edgelength(i)=sqrt(dx*dx+dy*dy)
	enddo
	edgeindex=(/ (i,i=1,nedgenew) /)		! indexical array
	!call Quicksort(edgelength,1,nedgenew,edgeindex,0)
	call KB07AI(edgelength,nedgenew,edgeindex)
	! print *,'       sorted  O_index'
 !   do i = 1,100
 !      print '(i3,1x,E10.3,1x,i8,1x,E10.3)',i,edgelength(i),edgeindex(i),edgelength(edgeindex(i)) 
 !   enddo
	vol=0.0
	do i=1,nedgenew
		...
		if(blanking(i).eq.0)then
			...
			if((edge(i,3).gt.0).and.(edge(i,4).gt.0))then
				...
				if((agg(edge(i,3)).eq.0).and.(agg(edge(i,4)).eq.0))then 
					! if the agg(regate) flag has not been raised for either of the cells
					! This is where i need to make changes 
					! I want it based on edge length
...
subroutine INTERSECT(pt_a,pt_b,ed_a,ed_b,dir,intsct,doesit)
	...
	double precision,intent(in),dimension(2)::pt_a,pt_b,ed_a,ed_b
	integer,intent(in)::dir
	double precision,intent(out),dimension(2)::intsct
...
!      double precision,intent(in),dimension(npt,2)::grdloc
!double precision,intent(out),dimension(maxsplits)::tempvol
...
!double precision,dimension(2)::base,pta,ptb
