...
function [snaxel,snakposition,snaxelmodvel,velcalcinfostruct,sensSnax,forceparam]...
    ...
    [snaxeltensvel,snakposition,velcalcinfostruct,sensSnax,forceparam]=...
...
function [snaxeltensvel,snakposition,velcalcinfostruct,sensSnax,forceparam]=...
    ...
    switch typeSmear
        case 'length'
            [derivtenscalc2]=ExtractDataForDerivatives_LengthSmear(snaxel,snakposition,...
                snakPosIndex,smearLengthEps);
        case 'lengthD'
            [derivtenscalc2]=ExtractDataForDerivatives_LengthDSmear(snaxel,snakposition,...
                snakPosIndex,smearLengthEps);
        case 'd'
            [derivtenscalc2]=ExtractDataForDerivatives_distanceSmear(snaxel,snakposition,...
                snakPosIndex,smearLengthEps,distEpsilon);
        case 'dlim'
            [derivtenscalc2]=ExtractDataForDerivatives_dlim(snaxel,snakposition,snakPosIndex,...
                smearLengthEps,distEpsilon);
        case 'dir'
            [derivtenscalc2]=ExtractDataForDerivatives_directionSmear(snaxel,snakposition,...
                snakPosIndex,smearLengthEps,distEpsilon,dirEpsilon);
        case 'dirturn'
            [derivtenscalc2]=ExtractDataForDerivatives_directionSmear2(snaxel,...
                snakposition,snakPosIndex,smearLengthEps,distEpsilon, dirEpsilon);
    end
    ...
    if forceparam.isLast
        [~,Hf,HA]=BuildJacobianAndHessian(derivtenscalc2,volumefraction,lagMulti,numel(volumefraction));
...
function [derivtenscalc]=ExtractDataForDerivatives_LengthSmear(snaxel,...
        snakposition,snakPosIndex,smearLengthEps)
...
function [derivtenscalc]=ExtractDataForDerivatives_LengthDSmear(snaxel,...
        snakposition,snakPosIndex,smearLengthEps)
    ...
    for ii=length(snakposition):-1:1
        ...
        derivtenscalc(ii).d_m=snaxel(neighSub).d;
        % calculating data
        smearVal = smearLengthEps^2*min(normVec(derivtenscalc(ii).Dg_i),...
            normVec(derivtenscalc(ii).Dg_m))^2;
        derivtenscalc(ii).normFi=sqrt(smearVal + sum(...
            (derivtenscalc(ii).p_i- derivtenscalc(ii).p_m).^2));
...
function [derivtenscalc]=ExtractDataForDerivatives_distanceSmear(snaxel,...
        snakposition,snakPosIndex,smearLengthEps,distEpsilon)
    normVec=@(vec) sqrt(sum(vec.^2,2));
    derivtenscalcTemplate=struct('index',[],...
        'snaxprec',[],...
        'precsub',[],...,...
...
function [derivtenscalc]=ExtractDataForDerivatives_directionSmear2(snaxel,...
        snakposition,snakPosIndex,smearLengthEps,distEpsilon, dirEpsilon)
    % added 07/05/2019 second try at direction smoothing for better vertex
    % crossing when L is very small mix in some of the neighbour
    normVec=@(vec) sqrt(sum(vec.^2,2));
    derivtenscalcTemplate=struct('index',[],...
        'snaxprec',[],...
        'precsub',[],...,...
    ...
    for ii=length(snakposition):-1:1
        ...
        dirMult = (1-derivtenscalc(ii).normFi/dirEpsilon);
        if(dirMult<=0)
            derivtenscalc(ii).Dg_i=(derivtenscalc(ii).Dg_i ...
                - dirMult*derivtenscalc(ii).Dg_m)/(1+dirMult);
            derivtenscalc(ii).Dg_m=(derivtenscalc(ii).Dg_m ...
                - dirMult*derivtenscalc(ii).Dg_i)/(1+dirMult);
        end
        %         derivtenscalc(ii).normFi=sqrt(smearLengthEps^2+...
        %             sum(((derivtenscalc(ii).g1_i+derivtenscalc(ii).Dg_i*...
        %             derivtenscalc(ii).d_i)-(derivtenscalc(ii).g1_m+...
...
function [derivtenscalc]=ExtractDataForDerivatives_dlim(snaxel,...
        snakposition,snakPosIndex,smearLengthEps,distEpsilon)
    normVec=@(vec) sqrt(sum(vec.^2,2));
    derivtenscalcTemplate=struct('index',[],...
        'snaxprec',[],...
    ...
    for ii=length(snakposition):-1:1
        ...
        [derivtenscalc(ii)]=CalculateDerivatives_d_lim(derivtenscalc(ii),smearLengthEps);
...
function [snakposition]=ModifySnakposition(snaxel,snakposition,snakPosIndex,eps)
    ...
    for ii=1:length(snakposition)
        ...
        distCoeff=(eps-edgeDist)/eps;
        distCoeff(distCoeff<0)=0;
...
%% Derivative calculations - Distance Smearing + Limit Taking

function [derivtenscalcII]=CalculateDerivatives_d_lim(derivtenscalcII,lSmear)
    ...
    [derivtenscalcII.dfiddi]=Calc_DFiDdi_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,d_i,d_m,lSmear,optNum);
    [derivtenscalcII.dfiddm]=Calc_DFiDdm_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,d_i,d_m,lSmear,optNum);
    
    if normFi==0
        normFi=1e-10;
    end
    
    [derivtenscalcII.d2fiddi2]=Calc_D2FiDdi2_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,d_i,d_m,lSmear);
    if abs(derivtenscalcII.d2fiddi2)>maxVal
        derivtenscalcII.d2fiddi2=maxVal*sign(derivtenscalcII.d2fiddi2);
    end
    
    [derivtenscalcII.d2fiddm2]=Calc_DFiDdm2_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,d_i,d_m,lSmear);
    if abs(derivtenscalcII.d2fiddm2)>maxVal
        derivtenscalcII.d2fiddm2=maxVal*sign(derivtenscalcII.d2fiddm2);
    end
    
    [derivtenscalcII.d2fiddim]=Calc_D2FiDdim_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,d_i,d_m,lSmear);
    if abs(derivtenscalcII.d2fiddim)>maxVal
        derivtenscalcII.d2fiddim=maxVal*sign(derivtenscalcII.d2fiddim);
    end
    
 end
% function [dfiddi]=Calc_DFiDdi_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,di,dm,lSmear,optNum)
%     
%     if normFi~=0
%         dfiddi=((2*a_i*di*(1-2*lSmear)+a_im*dm*(1-2*lSmear)+b_i*(1-2*lSmear))/(2*(normFi)));
%     else 
...
%             dfiddi=sqrt(a_i);
%         elseif optNum==2
%             dfiddi=a_im/(2*sqrt(a_m));
%         elseif optNum==3
%             dfiddi=(2*a_i+a_im)/(2*sqrt(a_i+a_m+a_im));
%         end
%     end
%     
%     
% end
% function [dfiddm]=Calc_DFiDdm_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,di,dm,lSmear,optNum)
%     
%     
%     if normFi~=0
%         dfiddm=(2*a_m*dm*(1-2*lSmear)+a_im*di*(1-2*lSmear)+b_m*(1-2*lSmear))/(2*(normFi));
...
%             dfiddm=sqrt(a_m);
%         elseif optNum==2
%             dfiddm=a_im/(2*sqrt(a_i));
%         elseif optNum==3
%             dfiddm=(2*a_m+a_im)/(2*sqrt(a_m+a_i+a_im));
%         end
%     end
% end
% function [d2fiddi2]=Calc_D2FiDdi2_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,di,dm,lSmear)
%     
%     
%     d2fiddi2=((2*a_i*(1-2*lSmear)*(1-2*lSmear)*2*(normFi)^2)...
%         -(2*a_i*di*(1-2*lSmear)+a_im*dm*(1-2*lSmear)+b_i*(1-2*lSmear))^2) ...
...
%     
% end
% function [d2fiddm2]=Calc_DFiDdm2_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,di,dm,lSmear)
%     
%     d2fiddm2=((2*a_m*(1-2*lSmear)*(1-2*lSmear)*2*(normFi)^2)...
%         -(2*a_m*dm*(1-2*lSmear)+a_im*di*(1-2*lSmear)+b_m*(1-2*lSmear))^2) ...
%         /(4*(normFi)^3);
% end
% function [d2fiddim]=Calc_D2FiDdim_d_lim(a_i,a_m,a_im,b_i,b_m,c,normFi,di,dm,lSmear)
%     
%     
%     d2fiddim=((a_im*(1-2*lSmear)*(1-2*lSmear)*2*(normFi)^2)...
%         -((2*a_m*dm*(1-2*lSmear)+a_im*di*(1-2*lSmear)+b_m*(1-2*lSmear))*(2*a_i*di*(1-2*lSmear)+a_im*dm*(1-2*lSmear)+b_i*(1-2*lSmear)))) ...
...
function [sensSnax,sensLagMulti]=CalculateSensitivity(Hf,Ha,HL,Ja_x,lagMulti,algo)
