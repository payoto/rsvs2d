...
function [J,g]=InverseDesign_ErrorTopoAD(profileComp,analysisLoop,targCoord)
    ...
    for ii=1:numel(analysisLoop)
        ...
        analysisLoop(ii).isanalysis=[nPrev+1:n]';
    end
    for ii=1:numel(targCoord)
        targCoord(ii).isanalysis=zeros(size(targCoord(ii).coord,1),1);
    end
    ...
    switch profileComp
        ...
        case 'area'
            [errorMeasure,modifiedDistance,indepLoop,funcArea]=CompareProfilesAreaTopo(analysisLoop,targCoord);
        case 'areasquared'
            [errorMeasure,modifiedDistance,indepLoop,funcArea]=CompareProfilesAreaSquaredTopo(analysisLoop,targCoord);
        case 'areadist'
            [errorMeasure,modifiedDistance,indepLoop,funcArea]=CompareProfilesAreaRawDistTopo(analysisLoop,targCoord);
        case 'areapdist'
            [errorMeasure,modifiedDistance,indepLoop,funcArea]=CompareProfilesAreaPRawDistTopo(analysisLoop,targCoord);
        case 'area2parea'
            [errorMeasure,modifiedDistance,indepLoop,funcArea]=CompareProfilesAreaSquaredpAreaTopo(analysisLoop,targCoord);
        case 'normdist'
...
function [errorMeasure,modifiedDistance,indepLoop,funArea]=CompareProfilesAreaTopo(testLoop,targLoop)
    % test intersection and internal with inpolygon
    % targLoop ->
    % testLoop |
    typeLoop='coord';
...
function [errorMeasure,modifiedDistance,indepLoop,funArea]=CompareProfilesAreaSquaredTopo(testLoop,targLoop)
    % test intersection and internal with inpolygon
    % targLoop ->
    % testLoop |
    typeLoop='coord';
...
function [errorMeasure,modifiedDistance,indepLoop,funArea]=CompareProfilesAreaSquaredpAreaTopo(testLoop,targLoop)
    % test intersection and internal with inpolygon
    % targLoop ->
    % testLoop |
    typeLoop='coord';
...
function [errorMeasure,modifiedDistance,indepLoop,funArea]=CompareProfilesAreaRawDistTopo(testLoop,targLoop)
    % test intersection and internal with inpolygon
    % targLoop ->
    % testLoop |
    typeLoop='coord';
...
function [errorMeasure,modifiedDistance,indepLoop,funArea]=CompareProfilesAreaPRawDistTopo(testLoop,targLoop)
    % test intersection and internal with inpolygon
    % targLoop ->
    % testLoop |
    typeLoop='coord';
...
function [J,g]=sensivFD(coord,OFModif,actind)
    %
    if ~exist('OFModif','var');OFModif=@(x)x;end
    % Run once
    [centreMat,normMat,ind]=CalculatePolyArea_prep(coord);
    J = OF(coord,OFModif,centreMat,normMat,ind);

    % Run finite differences
    delta = 1e-8;
    g = zeros(numel(actind),2);
    for j=1:2
        for ii =1:numel(actind)
            i=actind(ii);
            AF = coord;
            AF(i,j) = AF(i,j) + delta;
            dJ = OF(AF,OFModif,centreMat,normMat,ind) - J;
            g(ii,j) = dJ/delta;
...
function [J,g]=sensivAD(coord,OFModif,actind)
    %
    if ~exist('OFModif','var');OFModif=@(x)x;end
    % Run once
    [centreMat,normMat,ind]=CalculatePolyArea_prep(coord);
    ...
    J = OF(x,OFModif,centreMat,normMat,ind);
...
function [centreMat,normMat,ind]=CalculatePolyArea_prep(points)
...
function [A]=CalculatePolyArea_calc(points,centreMat,normMat,ind)
    
  
    A=0.5*(normMat*points(ind))'*(centreMat*points(ind));
...
function [indepLoop]=AreaErrorTopo(testLoop,targLoop)
    ...
    indepLoop=repmat(struct('coord',[],'isanalysis',[],'out',[]),[1 numel(indepTarg)+numel(indepTest)]);
    [indepLoop(:).coord]=deal(testLoop(indepTest).coord,targLoop(indepTarg).coord);
    [indepLoop(:).isanalysis]=deal(testLoop(indepTest).isanalysis,targLoop(indepTarg).isanalysis);
    ...
    for ii=1:size(testGroups,1)
        ...
        [newloop]=SeparateProfilesArea(testCurr(1).coord,targCurr(1).coord,...
            testCurr(1).isanalysis,targCurr(1).isanalysis);
...
function [newloop]=SeparateProfilesArea(profileCoord,targCoord,...
        profileCoordisanalysis,targCoordisanalysis)
    % compares two loops and returns the area not filled by both
    if ~exist('profileCoordisanalysis','var');profileCoordisanalysis=zeros([size(profileCoord,1),1]);end
    if ~exist('targCoordisanalysis','var');profileCoordisanalysis=zeros([size(targCoord,1),1]);end
    ...
    y0(rmv)=[];
    jout(rmv)=[];
    nprofC=size(profileCoord,1)+1;
    ntargC=size(targCoord,1)+1;
    iout(iout>=nprofC)=iout(iout>=nprofC)-nprofC+1;
    jout(jout>=ntargC)=jout(jout>=ntargC)-ntargC+1;
    
    [iout,sortIndex]=sort(iout);
    x0=x0(sortIndex);
    y0=y0(sortIndex);
    jout=jout(sortIndex);
    delInter=false(size(iout));
    for ii=2:numel(iout)
        delInter(ii)=iout(ii)==iout(ii-1) && jout(ii)==jout(ii-1);
    end
    iout(delInter)=[];
    jout(delInter)=[];
    x0(delInter)=[];
    ...
    if nX0>0
        newloop=repmat(struct('coord',zeros(0,2),'isanalysis',zeros(0,1)),[1,nX0]);
        nXplore=1:nX0;
        pLoop=0;
        while ~isempty(nXplore)
            ...
            while condLoop
                ...
                newloop(pLoop).coord=[newloop(pLoop).coord;
                    ...
                    [x0(iip1),y0(iip1)];
                    targCoord(ind2,:)];
                newloop(pLoop).isanalysis=[newloop(pLoop).isanalysis;
                    0;
                    profileCoordisanalysis(ind1);
                    0;
                    targCoordisanalysis(ind2)];
                kk=find(nXplore==iij);
