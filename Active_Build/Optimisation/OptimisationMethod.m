...
function [newPop,iterCurr]=OptimisationMethod(paramoptim,varargin)
    ...
    switch optimMethod
        ...
        case 'DESym'
            proj='tan';
            [newPop,iterCurr]=DifferentialEvolutionSym(paramoptim,proj,varargin{1},varargin{2});
        case 'GA'
...
function [newPop,iterCurr]=DifferentialEvolutionSym(paramoptim,proj,iterCurr,iterm1)
    switch proj
        case 'cut'
            projFunc=@(x) x;%(atan(x)/pi+1/2);
            projInv=@(x) x;%(tan((x-1/2)*pi));
