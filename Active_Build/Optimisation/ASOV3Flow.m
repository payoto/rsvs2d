function [objValue,additional]=ASOV3Flow(paramoptim,member,loop,baseGrid)
    ...
    ASOOptions = asoCase();
    ASOOptions.mach = nMach;
    ASOOptions.su2.parConf.np=currentMachineFile.slots;
    
    ASOOptions.su2.parConf.timeout = max(ceil(su2ProcSec/currentMachineFile.slots),60);
    ...
    if ~isempty(regexp(hostName,'4', 'once'))
        %ASOOptions.su2.mpiOpts=['-hosts ',currentMachineFile.node,' '];
    elseif ~isempty(regexp(hostName,'3', 'once'))
        ASOOptions.su2.parConf.hostfile = 'mpihostfile';
        ASOOptions.su2.parConf.cpuset = currentMachineFile.cpuset;
    end
...
%     end
    ...
    optimDirectory=['.'];
    ASOresult = asocases.su2DragMin(optimDirectory,ASOOptions);
