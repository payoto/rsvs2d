...
function [meshFile]=RemeshTriangle_Handler(paramoptim,boundaryLoc,varargin)
    ...
    [meshFile]=GenerateMesh(paramoptim,targFolder,varargin{:});
...
function [targFolder,boundaryLoc]=PrepareCFDFolder(paramoptim,boundaryLoc)
...
function [meshFile]=GenerateMesh(paramoptim,targFolder,varargin)
    ...
    if isLoad
        ...
        switch mesher
            ...
            case 'triangle'
                MakeTriangleMesh(paramoptim,targFolder,varargin{:});
                % flags meshing: triangle -pq33aDenV boundtriangle
                % flags remeshing: triangle -pq33aenV boundtriangle
                
                meshGenCommand=['"',targFolder,filesep,'RunTriangleSU2.',extStr,'"'];
                if numel(varargin)>1
                    % flags meshing: triangle -pq33aenV boundtriangle
                    meshGenCommand=['bash.exe "',targFolder,filesep,'RunTriangleSU2.sh" -pq33aenV '];
                end
    ...
    meshFile=[targFolder,filesep,'triangularmesh.su2'];
...
function []=MakeTriangleMesh(paramoptim,cfdDir,loop)
